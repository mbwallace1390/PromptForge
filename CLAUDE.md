...
- `promptforge.html` — the whole app. The `<script>` block is split into numbered sections:
  ...
  3. `DIMS` — the built-in question bank: one entry per thing a good build prompt should cover, with keyword `detect()` functions that pre-fill answers from the description
...
- `test/e2e.mjs` — Playwright smoke test that walks the whole flow in built-in mode and in AI mode against a mock OpenAI-compatible server on port 8787, plus the failure fallback and a phone-sized viewport. Screenshots land in `test/shots/` (git-ignored).
...
- Open `promptforge.html` in a browser to run it. `window.PromptForge` exposes `state`, `DIMS`, `buildStructuredPrompt`, and `settings` in the console.
