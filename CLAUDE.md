...
- "Free & local" is the `custom` provider with a `PRESETS` table (OpenRouter, Groq, Google AI Studio, Ollama, LM Studio, Other): each has a base URL, key page and help text; `settings.preset` is inferred from an older saved base URL by `presetFor()`. Never hard-code model names — `sortModels()` orders what "Fetch list" returns, free models first, and for OpenRouter the free ids are remembered in `settings.freeModels` and sent as OpenRouter's `models` fallback chain so a busy free model is skipped. `API.chat` retries any 429 once after 1.5 s. Cloud presets carry `needsKey`; `keyNeeded()` gates Test and Save with a plain message instead of a 401. `localOriginHint()` explains a failed local-server call for both the file case and the hosted case (it names the `OLLAMA_ORIGINS` value for the current origin).
